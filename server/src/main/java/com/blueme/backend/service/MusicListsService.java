...
public class MusicListsService {
	...
	public Long save(RecMusicListSaveDto requestDto) {
		...
		List<Musics> musics = requestDto.getRecommendedMusicIds().stream()
		...
		return recMusicListsJpaRepository.save(RecMusiclists.builder().user(user).title(requestDto.getTitle()).recMusicListDetail(recMusicListDetail).build()).getId();
	}
