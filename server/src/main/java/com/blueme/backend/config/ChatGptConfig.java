...
public class ChatGptConfig {
        ...
        public static final String QUESTION_TEMPLATE3 = "국가는 대한민국이고 현재온도는 섭씨%s이고 습도는 %s이고, 평균심박수는 %s bpm으로 %s 이고 속도는%skm/h이고 분당걸음수는 %s 으로 %s 인데, (id,title,artist,genre,tag) 형식으로 내가 노래리스트가 %s 형식으로 주어져있으며, 총 60곡이 있습니다. 제가 원하는것은 현재 건강데이터와 상황데이터에서 건강데이터와 상황데이터를 참고하되 노래의 태그를 우선순위로 가장 잘 어울리는 상위 10곡의 추천리스트입니다. JSON 형식으로 '전체적인 추천리스트 제목' 은 title로 하되 여러번 질문하더라도 안겹치게 다채롭고 문학적으로, '전체적인추천리스트 제목을 추천한이유'는 reason 으로 하되 계절정보는 제외하고 이유도 문학적으로, 그리고 musicIds배열을 포함해주세요. 추천이유를 자세히 설명하되 60자이내로, 내가 요구하지 않은 추가정보 없이 깔끔하게 작성해주세요.";
        public static final String QUESTION_TEMPLATE4 = "국가는 대한민국이고 평균심박수는 %s bpm으로 %s 이고 속도는%skm/h이고 분당걸음수는 %s 으로 %s 인데, (id,title,artist,genre,tag) 형식으로 내가 노래리스트가 %s 형식으로 주어져있으며, 총 60곡이 있습니다. 제가 원하는것은 현재 건강데이터와 상황데이터에서 건강데이터와 상황데이터를 참고하되 계절은 제외한 노래의 태그를 우선순위로 가장 잘 어울리는 상위 10곡의 추천리스트입니다. JSON 형식으로 '전체적인 추천리스트 제목' 은 title로 하되 여러번 질문하더라도 안겹치게 다채롭고 문학적으로, '전체적인추천리스트 제목을 추천한이유'는 reason 으로 하되 계절정보는 제외하고 이유도 문학적으로, 그리고 musicIds배열을 포함해주세요. 추천이유를 자세히 설명하되 60자이내로, 내가 요구하지 않은 추가정보 없이 깔끔하게 작성해주세요.";
